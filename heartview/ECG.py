...
class Filters:
    ...
    def muscle_noise(
        ...
        """
        if lowcut <= 0 or highcut >= self.fs / 2 or lowcut >= highcut:
            raise ValueError('Invalid cutoff frequencies: `lowcut` must be > 0 ' 
                             'and < `highcut`, and `highcut` must be < Nyquist '
                             'frequency.')
        nyquist = 0.5 * self.fs
        low = lowcut / nyquist
    ...
    def filter_signal(
        ...
        ----------
        signal : np.ndarray
            A NumPy array containing the ECG data to be filtered.
        lowcut : float, optional
            The lower cutoff frequency of the bandpass filter; by default, 
            1.0 Hz.
        highcut : float, optional
            The upper cutoff frequency of the bandpass filter; by default, 
            15.0 Hz.
        rs : float, optional
            The minimum stopband attenuation in dB; by default, 0.15 dB.
        rp : float, optional
            The maximum passband ripple in dB; by default, 80.0 dB.
        order : int, optional
        ...
        ------
        ValueError
            If `lowcut` or `highcut` is outside the valid range.
        """
        if lowcut <= 0 or highcut >= self.fs / 2 or lowcut >= highcut:
            raise ValueError('Invalid cutoff frequencies: `lowcut` must be > 0 ' 
                             'and < `highcut`, and `highcut` must be < Nyquist '
...
class BeatDetectors:
    ...
    def engzee(self, signal: Union[np.ndarray, list]) -> np.ndarray:
        ...
        ecg_beats = self.__remove_dupes(ecg_beats)
        return ecg_beats
    ...
    def manikandan(
        ...
        def _adapt_thresh(
            ...
            return np.array(passing_beats, dtype = np.int64)
        ...
        sn_f = np.insert(self._ma(sn, window_len),
                         0, [0] * (window_len - 1))
        ...
        zn_ma = np.insert(
            self._ma(zn, ma_len), 0, [0] * (ma_len - 1))
    ...
    def _ma(
        ...
        signal: Union[np.ndarray, list], 
        window_len: int, 
        method: Literal['cumsum', 'convolution'] = 'cumsum'
    ) -> np.ndarray:
        """Applies a moving average filter using either cumulative sum 
        or convolution."""
        if method not in ('cumsum', 'convolution'):
            raise ValueError('The `method` parameter must be either '
                             '\'cumsum\' or \'convolution\'.')
        if method == 'cumsum':
            ...
            ma = np.convolve(
                padded_signal, np.ones(window_len) / window_len, mode = 'valid')
        return ma

    def _butter_bandpass_filter(
        self, 
        signal: Union[np.ndarray, list], 
        method: Literal['pan', 'hamilton'],
    ) -> np.ndarray:
    ...
    def __remove_dupes(
        self, 
        ecg_beats: Union[np.ndarray, list]
    ) -> np.ndarray:
